...
- `scripts/transcribe-file.sh` (`scripts/transcribe-file.ps1` on native Windows): sends the first chunk of a WAV to the configured inference endpoint and prints the result; quickest way to check a model setup.
