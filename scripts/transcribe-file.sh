...
endpoint="${INFERENCE_URLS:-http://localhost:8080}"
endpoint="${endpoint%%,*}"
endpoint="${endpoint//[[:space:]]/}"
endpoint="${endpoint%/}"
if [[ ! "$endpoint" =~ ^https?://[^/]+ ]]; then
  echo "INFERENCE_URLS must start with an HTTP(S) endpoint" >&2
  exit 2
fi
...
# Gemma 4 accepts at most 30 seconds of audio per request. Ten seconds is a
# representative first chunk and keeps this check quick on a new machine.
...
if ! request_time="$(curl --fail-with-body --silent --show-error --max-time 180 \
  ...
  echo "Inference request failed at $endpoint/v1/chat/completions" >&2
